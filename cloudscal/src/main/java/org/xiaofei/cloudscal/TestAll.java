...
public class TestAll {
	...
	public static void main(String[] args) {
		Keystone keystone = new Keystone(CeilometerConfiguration.KEYSTONE_AUTH_URL);
		Access access = keystone.tokens()
				.authenticate(new UsernamePassword(CeilometerConfiguration.KEYSTONE_USERNAME, CeilometerConfiguration.KEYSTONE_PASSWORD))
				.withTenantName("demo")
				.execute();
		
		Ceilometer ceilometer = new Ceilometer(CeilometerConfiguration.CEILOMETER_ENDPOINT);
		ceilometer.setTokenProvider(new OpenStackSimpleTokenProvider(access.getToken().getId()));

		OpenStackRequest<Ceilometer> openStackRequest=new OpenStackRequest<Ceilometer>();
		List<Resource> resources = ceilometer.execute(new Resource().eq("resource_id", "23b55841eedd41e99d5f3f32149ca086"));
